...
describe("production procedural river consumer integration", () => {
  it("feeds water, refined terrain, bridges, POIs, objects, and gameplay from the active spine", () => {
    const seed = 7, riverChunk = riverChunkAtProgress(.5), data = generateChunk(seed, riverChunk);
    expect(data.irregularTerrain, `missing refined terrain in ${JSON.stringify(riverChunk)}`).toBeDefined();
    expect(tessellateWorldRiverWaterChunk(riverChunk).vertices.length).toBeGreaterThan(0);
    ...
    const point = riverPointAtProgress(.5), bounds = { minX: point.x - 8, maxX: point.x + 8, minZ: point.z - 8, maxZ: point.z + 8 };
    const gameplay = sampleWorldRiverGameplay(seed, point.x, point.z, createWorldRiverGameplayContext(bounds));
