...
import { createActiveTerrainSurfaceIndex, queryChunkTerrainSurface } from "./activeTerrainSurface";
...
function run(label: string, seed: string, coordinate: { x: number; z: number }): void {
  ...
  void createActiveTerrainSurfaceIndex(chunk);
  ...
  console.info(`[terrain-query] ${label}: median=${percentile(queryMs, .5).toFixed(4)}ms p95=${percentile(queryMs, .95).toFixed(4)}ms candidatesMedian=${percentile(candidates, .5)} candidatesP95=${percentile(candidates, .95)} indexBuild=${indexBuildMs.toFixed(4)}ms chunkGeneration=${timings?.totalMs.toFixed(3)}ms vertices=${chunk.terrainMesh.positions.length / 3} triangles=${chunk.terrainMesh.indices.length / 3} estimatedCpuQueryBytes=${chunk.terrainSurfaceIndex.estimatedBytes}`);
}
