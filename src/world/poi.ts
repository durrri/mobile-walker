...
export interface GeneratedPoi {
  ...
  readonly navigationAnchor:Readonly<{x:number;y:number;z:number;kind:"entrance"|"dock-landing"|"centre"}>;
...
export function generatePois(seedInput:number|string,coordinate:ChunkCoordinate):Readonly<{pois:readonly GeneratedPoi[];candidates:readonly PoiDebugCandidate[]}>{
  ...
  const riverContext=createWorldRiverRelationshipContext({minX:(baseX-maximumMargin)*CELL_SIZE,maxX:(endX+maximumMargin+1)*CELL_SIZE,minZ:(baseZ-maximumMargin)*CELL_SIZE,maxZ:(endZ+maximumMargin+1)*CELL_SIZE},16);
