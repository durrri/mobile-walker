...
export class RiverSpine {
  ...
  constructor(points: readonly RiverControlPoint[], options: RiverSpineOptions = {}) {
    ...
    this.bounds = Object.freeze(segments.reduce((value, segment) => ({
      minX: Math.min(value.minX, segment.bounds.minX), maxX: Math.max(value.maxX, segment.bounds.maxX),
      minZ: Math.min(value.minZ, segment.bounds.minZ), maxZ: Math.max(value.maxZ, segment.bounds.maxZ),
...
export const AUTHORED_R6_RIVER_CONTROL_POINTS: readonly RiverControlPoint[] = Object.freeze([
...
/** Permanent R6 comparison fixture. */
export const authoredR6RiverSpine = new RiverSpine(AUTHORED_R6_RIVER_CONTROL_POINTS);
