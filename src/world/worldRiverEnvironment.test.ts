...
const context = createWorldRiverEnvironmentContext({ minX: -20, maxX: 20, minZ: -20, maxZ: 20 }, spine,createRiverWidthProfile("environment",spine));
const half=context.carving.widthProfile.sampleAtProgress(.5).halfWidth,lip=half+WORLD_RIVER_CARVING.shoreTransitionWidth;
...
describe("world river environmental zones", () => {
  it("classifies the authoritative cross-section landmarks symmetrically", () => {
    ...
    expect(at(half).zone).toBe("water");
    expect(at(half + 0.01).zone).toBe("shoreTransition");
    expect(at(half+WORLD_RIVER_CARVING.shoreTransitionWidth + 0.01).zone).toBe("walkableBank");
    expect(at(half + WORLD_RIVER_CARVING.bankWidth + 0.01).zone).toBe("outerFalloff");
    expect(at(half+WORLD_RIVER_CARVING.bankWidth+WORLD_RIVER_CARVING.falloffWidth + 0.01).zone).toBe("outsideRiverInfluence");
  ...
  it("is orientation independent on east/west and diagonal reaches", () => {
    for (const points of [
      ...
      const local = createWorldRiverEnvironmentContext({ minX: -10, maxX: 10, minZ: -10, maxZ: 10 }, oriented,createRiverWidthProfile("environment-oriented",oriented));
      const frame = oriented.sampleFrame(0.5);
      const localLip=local.carving.widthProfile.sampleAtProgress(.5).halfWidth+WORLD_RIVER_CARVING.shoreTransitionWidth;
      const sample = sampleWorldRiverEnvironment(
        frame.position.x + frame.normal.x * (localLip + 0.1),
        frame.position.z + frame.normal.z * (localLip + 0.1), local,
      );
  ...
  it("remains deterministic on a strong bend and across overlapping contexts", () => {
    ...
    const profile=createRiverWidthProfile("environment-bend",bend);
    const a = createWorldRiverEnvironmentContext({ minX: -12, maxX: 4, minZ: -8, maxZ: 8 }, bend,profile);
    const b = createWorldRiverEnvironmentContext({ minX: -4, maxX: 12, minZ: -8, maxZ: 8 }, bend,profile);
  ...
  it("takes a no-river fast path for dry bounds", () => {
    const dry = createWorldRiverEnvironmentContext({ minX: 100, maxX: 110, minZ: 100, maxZ: 110 }, spine,context.carving.widthProfile);
...
describe("river object category policy", () => {
  ...
  it.each(["tree", "largeShrub", "largeRock", "decorativeProp"] as const)("clears %s through the walkable bank", category => {
    ...
    expect(decision(category, half + 0.1).accepted).toBe(false);
    expect(decision(category, lip + 0.1).accepted).toBe(false);
  });

  it("allows bank flowers and reachable collectibles but never water collectibles", () => {
    expect(decision("tinyVegetation", lip + 0.1).accepted).toBe(true);
    expect(decision("collectible", lip + 0.3).accepted).toBe(true);
    expect(decision("wetlandPool", lip + 0.8).accepted).toBe(true);
    expect(decision("collectible", 0).accepted).toBe(false);
    expect(decision("wetlandPool", half + 0.1).accepted).toBe(false);
  });

  it("uses stable category hashes for shoreline flowers and bank rocks", () => {
    const repeated = Array.from({ length: 100 }, (_, identity) => decision("tinyVegetation", half+.1, identity).accepted);
    expect(repeated).toEqual(Array.from({ length: 100 }, (_, identity) => decision("tinyVegetation", half+.1, identity).accepted));
  ...
  it("delegates every category unchanged throughout the outer falloff", () => {
    const outer = half + WORLD_RIVER_CARVING.bankWidth + 0.7;
  ...
  it("uses category footprints symmetrically and across context boundaries", () => {
    const centre = lip + RIVER_OBJECT_CLEARANCE.tree - 0.01;
    ...
    const left = createWorldRiverEnvironmentContext({ minX: -8, maxX: 0, minZ: -8, maxZ: 8 }, spine,context.carving.widthProfile);
    const right = createWorldRiverEnvironmentContext({ minX: 0, maxX: 8, minZ: -8, maxZ: 8 }, spine,context.carving.widthProfile);
