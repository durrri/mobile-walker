...
export interface BeaconPresentationProfile {
  ...
  readonly lightIntensity: number; readonly farVisibility: number; readonly smoke?: BeaconSmokeProfile;
}
...
export const BEACON_PRESENTATION_PROFILES: Readonly<Record<PoiBeaconProfile, Readonly<Record<PoiBeaconFixtureKind, BeaconPresentationProfile>>>> = Object.freeze({
  homestead: Object.freeze({ lantern: Object.freeze({ priority: 1, glowScale: .28, lightRange: 7, lightIntensity: 1.25, farVisibility: 1 }), fire: Object.freeze({ priority: 1.1, glowScale: .5, lightRange: 8, lightIntensity: 1.5, farVisibility: 1, smoke: Object.freeze({ puffs: 3, height: 3.8, scale: .65 }) }) }),
  waterside: Object.freeze({ lantern: Object.freeze({ priority: 1.1, glowScale: .3, lightRange: 8, lightIntensity: 1.35, farVisibility: 1.1 }), fire: Object.freeze({ priority: 1.1, glowScale: .5, lightRange: 8, lightIntensity: 1.5, farVisibility: 1.1, smoke: Object.freeze({ puffs: 3, height: 4, scale: .7 }) }) }),
  cabin: Object.freeze({ lantern: Object.freeze({ priority: .9, glowScale: .26, lightRange: 7, lightIntensity: 1.2, farVisibility: .9 }), fire: Object.freeze({ priority: 1, glowScale: .48, lightRange: 8, lightIntensity: 1.45, farVisibility: 1, smoke: Object.freeze({ puffs: 3, height: 3.6, scale: .62 }) }) }),
  "regional-watchtower": Object.freeze({ lantern: Object.freeze({ priority: 1.5, glowScale: .34, lightRange: 9, lightIntensity: 1.5, farVisibility: 1.4 }), fire: Object.freeze({ priority: 1.7, glowScale: .58, lightRange: 10, lightIntensity: 1.8, farVisibility: 1.5, smoke: Object.freeze({ puffs: 3, height: 4.6, scale: .8 }) }) }),
});
...
export class PoiBeaconPresentation {
  ...
  constructor(scene:THREE.Scene, private readonly state:Pick<PoiBeaconState,"getState">, maximumLights=BeaconLightManager.DEFAULT_BUDGET) { this.root.name="poi-beacons";scene.add(this.root);this.lightManager=new BeaconLightManager(this.root,maximumLights); }
  activatePoi(poi:GeneratedPoi): readonly BeaconFixturePresentation[] {
    ...
    this.handles.set(poi.id,{poi,fixtures,disposed:false});return fixtures;
  }
